...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate, ItemSelectionViewControllerDelegate {
    ...
    private func addPhotoOutput() {
        if session.canAddOutput(photoOutput) {
            ...
            photoOutput.maxPhotoDimensions = CMVideoDimensions(width: 1024, height: 768)
    ...
    @IBAction private func toggleCaptureMode(_ captureModeControl: UISegmentedControl) {
        ...
        if captureModeControl.selectedSegmentIndex == CaptureMode.photo.rawValue {
            ...
            sessionQueue.async {
                ...
                self.enableButtonsBasedOnPhotoOutput()
    ...
    private func enableButtonsBasedOnPhotoOutput() {
    ...
    @IBAction private func capturePhoto(_ photoButton: UIButton) {
        ...
        sessionQueue.async {
            ...
            let photoCaptureProcessor = self.createPhotoCaptureProcessor(with: photoSettings)
    ...
    private func createPhotoSettings() -> AVCapturePhotoSettings {
        ...
        photoSettings.maxPhotoDimensions = CMVideoDimensions(width: 1024, height: 768)
    ...
    private func createPhotoCaptureProcessor(with photoSettings: AVCapturePhotoSettings) -> PhotoCaptureProcessor {
        PhotoCaptureProcessor(with: photoSettings, willCapturePhotoAnimation: {
    ...
    @IBAction private func toggleLivePhotoMode(_ livePhotoModeButton: UIButton) {
        sessionQueue.async {
            ...
            DispatchQueue.main.async {
                if livePhotoMode == .on {
                    self.livePhotoModeButton.setImage(TimelineUIAsset.livePhotoON.image, for: [])
                } else {
                    self.livePhotoModeButton.setImage(TimelineUIAsset.livePhotoOFF.image, for: [])
                }
            }
        }
    }
    ...
    @IBAction func toggleDepthDataDeliveryMode(_ depthDataDeliveryButton: UIButton) {
        sessionQueue.async {
            ...
            DispatchQueue.main.async {
                if depthDataDeliveryMode == .on {
                    self.depthDataDeliveryButton.setImage(TimelineUIAsset.depthON.image, for: [])
                    self.portraitEffectsMatteDeliveryButton.setImage(TimelineUIAsset.portraitMatteON.image, for: [])
                    self.semanticSegmentationMatteDeliveryButton.isEnabled = true
                } else {
                    self.depthDataDeliveryButton.setImage(TimelineUIAsset.depthOFF.image, for: [])
                    self.portraitEffectsMatteDeliveryButton.setImage(TimelineUIAsset.portraitMatteOFF.image, for: [])
...
//                movieFileOutput.startRecording(to: URL(fileURLWithPath: outputFilePath), recordingDelegate: self)
    ...
    func fileOutput(_ output: AVCaptureFileOutput, didStartRecordingTo fileURL: URL, from connections: [AVCaptureConnection]) {
        ...
        DispatchQueue.main.async {
            self.recordButton.isEnabled = true
            self.recordButton.setImage(TimelineUIAsset.captureStop.image, for: [])
        }
    }
